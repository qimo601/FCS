#include "PolygonPicker.h"

//矩形设门
//提供选择类
PolygonPicker::PolygonPicker(QWidget *canvas)
...
//将位置转换成字符串
QwtText PolygonPicker::trackerTextF(const QPointF &pos) const
{
	QwtText text;
...
void PolygonPicker::move(const QPoint & pos)
{
	QwtPlotPicker::move(pos);
}
void PolygonPicker::stretchSelection(const QSize &oldSize, const QSize &newSize)
{
...
bool PolygonPicker::end(bool ok)
{
 ...
 void PolygonPicker::reset()
{
	 QwtPlotPicker::reset();
}
 void PolygonPicker::remove()
{
