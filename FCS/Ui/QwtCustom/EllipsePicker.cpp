#include "EllipsePicker.h"

//椭圆设门
//提供选择类
EllipsePicker::EllipsePicker(QWidget *canvas)
...
EllipsePicker::~EllipsePicker()
{

}
//将位置转换成字符串
QwtText EllipsePicker::trackerTextF(const QPointF &pos) const
{
	QwtText text;
...
void EllipsePicker::move(const QPoint & pos)
{
	QwtPlotPicker::move(pos);
}
void EllipsePicker::stretchSelection(const QSize &oldSize, const QSize &newSize)
{
...
bool EllipsePicker::end(bool ok)
{
 ...
 void EllipsePicker::reset()
{
	 QwtPlotPicker::reset();
}
 void EllipsePicker::remove()
{
